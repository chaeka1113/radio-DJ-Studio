...
## 🚨 テンキ爺 라디오 프로젝트 전용 강제 규칙 (V3 오디오 태그 포맷팅)

### 규칙 1: 태그 앞뒤 공백 필수

V3 오디오 태그(`[sighs]`, `[scoffs]`, `[laughs]` 등)는 **반드시 앞뒤로 공백(Space)을 한 칸 이상** 두어야 합니다.
태그가 인접 문자와 붙으면 TTS 엔진이 태그를 텍스트로 병합하거나 발음이 부자연스러워집니다.

| | 예시 |
|---|---|
| ❌ 잘못된 형식 | `だと？[scoffs]老いぼれ` |
| ⭕ 올바른 형식 | `だと？ [scoffs] 老いぼれ` |

**추가 예시:**
- ❌ `コノヤロー！[sighs]まったく` → ⭕ `コノヤロー！ [sighs] まったく`
- ❌ `そうか[laughs]それは` → ⭕ `そうか [laughs] それは`
- ❌ `[scoffs]何を言ってる` → ⭕ `[scoffs] 何を言ってる`

### 규칙 2: 문장 부호 뒤 태그 — 말줄임표로 휴지 강화 (권장)

문장 부호(。！？) 뒤에 오디오 태그를 쓸 경우, 태그 앞뒤로 말줄임표(`...`)를 추가하면 자연스러운 호흡 휴지를 만들 수 있습니다.
...
대본 생성 후 TTS 전달 전 반드시 아래를 확인하십시오:

- [ ] 모든 `[태그]` 앞뒤로 공백이 있는가?
- [ ] 태그가 일본어 문자(ひらがな・カタカナ・漢字)에 직접 붙어 있지 않은가?
- [ ] 태그가 구두점(。！？、)에 바로 붙어 있지 않은가 (공백 또는 `...` 사이에 있는가)?
