...
아래 "기존 오답 노트"와 "이번 회차 신규 실수 목록"을 받아 카테고리별로 분류된 최종 오답 노트를 만들어라.
...
6. 아래 카테고리 HTML 주석 헤더를 사용하여 분류하라 (카테고리당 최대 3개 bullet):
   - <!-- CAT:V3태그위반 --> : 일본어 지문·Audio Tag·SSML 관련 오류
   - <!-- CAT:계약불이행 --> : 키워드 미포함·연령·빌런 플래그 오류
   - <!-- CAT:수치오류 --> : 가격·연금·시급 등 수치 범위 오류
   - <!-- CAT:서사품질 --> : 지명 없음·대화 인용 없음·テンキ爺 후크 없음
   - <!-- CAT:분량 --> : 800자 미만·1000자 초과 오류
   - <!-- CAT:기타 --> : 위에 해당 없는 오류
7. 카테고리 헤더 다음 bullet list, 코드블록·JSON·부연설명 없이 텍스트만 반환하라.

## 출력 형식 예시
<!-- CAT:V3태그위반 -->
- [V3태그] script 필드에 [sighs] 영문 태그 사용 금지 — 사연자 편지에서 지문은 완전 삭제하거나 ...로 대체.
<!-- CAT:수치오류 -->
- [수치] おにぎり 가격은 140〜180円 — 100円이나 300円은 실제 범위 벗어남.
...
## 최종 오답 노트 (카테고리 헤더 + ${MAX_BULLETS}개 이하 bullet만 출력):`;
